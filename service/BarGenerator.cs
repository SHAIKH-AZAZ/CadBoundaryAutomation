...
namespace CadBoundaryAutomation.Services
{
    ...
    {
        // Key format: "Horizontal|1465.58"
        private static string MakeKey(string orientation, double roundedLen)
        {
            return orientation + "|" + roundedLen.ToString("F2", CultureInfo.InvariantCulture);
        }

        private static void AddToGroups(
            Dictionary<string, BarJson> groups,
            string orientation,
            double roundedLen,
            string handle
        )
        {
            string key = MakeKey(orientation, roundedLen);

            if (!groups.TryGetValue(key, out BarJson g))
            {
                g = new BarJson
                {
        ...
        public static void GenerateHorizontalBars(
            ...
            Dictionary<string, BarJson> groupedBars
        )
        {
            for (double y = minY; y <= maxY; y += spacing)
            {
                ...
                ))
                {
                    ...
                    for (int i = 0; i < usable - 1; i += 2)
                    {
                        ...
                        // ✅ Rounded length in JSON
                        double len2 = Math.Round(len, 2, MidpointRounding.AwayFromZero);

                        // ✅ Group by (Orientation + rounded length)
                        AddToGroups(groupedBars, "Horizontal", len2, bar.Handle.ToString());

                        ed.WriteMessage($"\nH-Bar {barIndex}: {len2:F2} mm");
                        barIndex++;
                    }
                }
            }
            ...
            for (double x = minX; x <= maxX; x += spacing)
            {
                ...
                ))
                {
                    ...
                    for (int i = 0; i < usable - 1; i += 2)
                    {
                        ...
                        AddToGroups(groupedBars, "Vertical", len2, bar.Handle.ToString());

                        ed.WriteMessage($"\nV-Bar {barIndex}: {len2:F2} mm");
                        barIndex++;
                    }
                }
            }
